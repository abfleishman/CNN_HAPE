...
head(a)
file.copy(a$Path,paste0("D:/CM,Inc/CNN_HAPE/Deploy_data/",basename(a$Path)))
head(a)
...
for(i in 1:length(a$Project)){
  ...
  data.list1[[i]]<-data.list1[[i]] %>% select(one_of("SPID","Sensor_Name","Deployment_Date","Deployment_Time","Retrieval_Date","Retrieval_Time","Easting","Northing","Latitude","Longitude",'Island',"Site_Name")) %>% mutate_all(.funs = as.character)
